...
def useWappalyzer(url):
    try:
        ...
        webprints = wappalyzer.analyze(webpage) # 指纹识别
        if len(webprints) > 0:
    ...
    if len(sys.argv) < 2:
        ...
        try:
            ...
            if re.match(r'^https?:/{2}\w.+$', target_url):
                ...
                cms = Cmsscanner(target_url) # 进行cms扫描器（tide库）
                fofa_finger = cms.run()
                banner = []
                for x in fofa_finger:
                    banner.append(x)
                try:
                    Wappalyzer = useWappalyzer(target_url) # 进行wappalyzer扫描
                ...
                try:
                    update = False
                    webanalyzer_banner = webanalyzer.check(target_url,update) # 进行webanalyzer扫描
